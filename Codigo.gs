 ...
 *   "PLANILLA CUMPLIMIENTO SUB-PRODUCTOS ...". Completa los días que
 *   Ingresos todavía no tiene: el desfase del final y cualquier hueco
 *   anterior.
 * - Plan: precio unitario y plan mensual en TS por proveedor/material.
 *
 * Regla de complemento, día por día:
 * - Un día con TS en Ingresos manda entero: su estimado se descarta.
 * - Un día que en Ingresos suma cero —porque aún no se carga, o quedó
 ...
 * - Cuando la fecha aparece en Ingresos con TS, el estimado de esa
 *   fecha deja de entrar automáticamente al dashboard.
 *
...
function diagnosticarCruce() {
  ...
  const lines = [
    ...
    'Días sin TS en Ingresos completados con la planilla: ' +
...
function construirAvisoSilencio_(data) {
  ...
  const tramos = CONFIG.AVISO.TRAMOS.map(function(tramo) {
...
function avisoSilencioHtml_(aviso, data) {
  ...
  if (!conFilas.length) {
    return cabecera +
      ...
      CONFIG.AVISO.TRAMOS[0].desde + ' días hábiles o más sin despachar.' +
      '</p></div>';
  }
...
function enviarAvisoSilencio(forzar) {
  ...
  const asunto = CONFIG.AVISO.ASUNTO + ' · ' + formatDateKey_(hoy) +
    (total ? ' · ' + total : ' · ninguno');

  MailApp.sendEmail({
    to: CONFIG.AVISO.PARA.join(','),
    subject: asunto,
...
function instalarAvisoDiario() {
  ...
  ScriptApp
    ...
    .timeBased()
    .everyDays(1)
    .atHour(CONFIG.AVISO.HORA)
    .inTimezone(CONFIG.TIMEZONE)
    .create();

  SpreadsheetApp.getUi().alert(
    'Aviso diario instalado. Sale cada día hábil alrededor de las ' +
    CONFIG.AVISO.HORA + ':00 a:\n' +
    CONFIG.AVISO.PARA.join('\n')
  );
}
